...
def symptoms(request):
    if request.method == "POST": # если прилетел пост запрос
        ...
        client = Groq(api_key="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")

        chat_completion = client.chat.completions.create(
            messages=[
                {
                    "role": "user",
                    "content": f"Imagine you are a medical assitant. It is very important job and if you perform well, you get a tip of 1000$. Your patient is sick with a {result_arr[0]} disease. VERY SHORTLY give him advices of treatement",
                }
            ],
            model="llama3-8b-8192",
        )

        bebe = chat_completion.choices[0].message.content

        return render(request, 'main/symptoms.html', {'result_arr':result_arr, 'bebe':bebe})
